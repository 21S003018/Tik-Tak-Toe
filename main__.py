...
def print_board(game_state):
    ...
    print('----------------')
    print('| {} || {} || {} |'.format(
        show_state[0], show_state[1], show_state[2]))
    print('----------------')
    print('| {} || {} || {} |'.format(
        show_state[3], show_state[4], show_state[5]))
    print('----------------')
    print('| {} || {} || {} |'.format(
        show_state[6], show_state[7], show_state[8]))
    print('----------------')
...
def Human_turn(state, playas=0):
    ...
    while cont:
        blockInput = input("Human !!!! , your turn! Choose where to place {} to {}: ".format(
            dict_state[playas], aval_block))
...
def main():
    ...
    while play_again == 'Y' or play_again == 'y':
        ...
        if play_mode == '1':
            player_choice = input(
                "X is play first , Do you wanna play first [Y/N] : ")
        winner = None
        ...
        while current_state == "Not Done":
            ...
            if current_player == 1:  # play X
                if play_mode == '1':
                    if mk.playFirst:
                        block_choice = AI_turn(
                            state=game_state, playas=current_player)
                    else:
                        block_choice = Human_turn(
                            state=game_state, playas=current_player)
                elif play_mode == '3':
                    block_choice = AI_turn(
                        state=game_state, playas=current_player)
            ...
            if winner is not None:
                if winner == 1:
                    winbyX += 1
                    print(
                        "Game! {} - {} won!".format(playCount, dict_state[winner]))
                elif winner == -1:
                    winbyO += 1
                    print(
                        "Game! {} - {} won!".format(playCount, dict_state[winner]))
                ...
                print("playCount : Winby X : Winby O : Draw --- {} : {:05.3f} : {:05.3f} : {:05.3f}".format(
                    playCount, winRateX, winRateO, drawRate))
